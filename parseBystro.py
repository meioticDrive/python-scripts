...
def main():
    ...
    #Print column titles
    print("Chromosome\t", "Position\t", "Type\t", "phastCons\t", "phyloP\t", "cadd\t", "dbSNP.name\t", "dbSNP.func\t", "gnomad.genomes.id\t", "gnomad.genomes.af\t", "refSeq.siteType\t", "refSeq.exonicAllelefunction\t", "refSeq.description\t", file=outfile)
    ...
    for line in infile:
        ...
        #print(line[:-1])
        temp = line.split('\t')
        print(temp[0], "\t", temp[1], "\t", temp[2], "\t", temp[45], "\t", temp[46], "\t", temp[47], "\t", temp[48], "\t", temp[52], "\t", temp[66], "\t", temp[67], "\t", temp[16], "\t", temp[17], "\t", temp[30], "\t", file=outfile)
